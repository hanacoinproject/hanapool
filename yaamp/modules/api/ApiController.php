...
class ApiController extends CommonController
{
    ...
    public function actionHLP() {
		$coinId = 1500;
		$memcacheStr = "$coinId-hlp";
		$json = controller()->memcache->get($memcacheStr);
		...
		$stats = array(
            ...
            "height" => $height,
			"total_supply" => $this->getTotalSupply($height)
		);

		$json = json_encode($stats);

		controller()->memcache->set($memcacheStr, $json);

		echo $json;
		return;
	}

	private function getTotalSupply($height) {
		$halving_interval = 1225000;
		$initial_reward = 50;
		$eras = floor($height / $halving_interval);
		$rewards_prev_eras = $halving_interval * $initial_reward * (1 - pow(0.5, $eras));
		$rewards_curr_era = ($height - ($eras * $halving_interval) + 1) * $initial_reward * pow(0.5, $eras);
		$adjustments = 0;
		return $rewards_prev_eras + $rewards_curr_era + $adjustments;
	}
